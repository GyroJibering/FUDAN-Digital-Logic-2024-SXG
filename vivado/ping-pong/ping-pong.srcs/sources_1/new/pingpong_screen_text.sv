module pingpong_screen_text(
    ...
    assign rowAddr = yText[3:0];  // 每个字共16行
    ...
    // 提取出每个字符一行中的每1位数据
    assign One_bit = textData[xText[2:0]];

    // 16x8点阵 "Score:??"  
    assign textOn = ((pix_x > H_SYNC_START+C2)     &&
                     (pix_x < H_SYNC_START+C2+8*8) && //8字符*8bits
                     (pix_y > V_SYNC_START+R2)     && 
                     (pix_y < V_SYNC_START+R2+16)) && //16bits高
                     One_bit ;  // 像素=1？
